...
function seedTestData(db: DatabaseSync) {
  ...
  const stmt = db.prepare(
    `INSERT INTO runs (ts, role, model, acceptance, completion, tokens_in, tokens_out, cost, source, template_id, session_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  );

  // template-a data
  stmt.run(now, "coder", "model-A", "accepted", 1200, 4000, 2000, 0.003, "scheduler", "template-a", "sess-1");
  stmt.run(now - 60000, "coder", "model-A", "accepted", 1100, 3500, 1800, 0.002, "scheduler", "template-a", "sess-1");
  stmt.run(now - 120000, "coder", "model-B", "rejected", 2500, 5000, 2500, 0.01, "scheduler", "template-a", "sess-1");
...
describe("telemetry", () => {
  ...
  it("aggregateByRole filters by templateId", () => {
    const rows = aggregateByRole(db, undefined, "template-a");
    // template-a: 3 runs + NULL template_id: 1 run
    expect(rows.length).toBeGreaterThanOrEqual(1);
    // All should belong to template-a or be legacy
    const coderModelA = rows.find((r) => r.role === "coder" && r.model === "model-A");
    expect(coderModelA).toBeDefined();
    // template-a model-A: 2 runs + legacy model-A: 1 run = 3
  ...
  it("listRoles filters by templateId", () => {
