...
> 2026-08-09 · v0.8 循环①——v0.9 交互层升级（不自己开发任务、job 提交后脱手）的基础。

## 用意

v0.9 升级 PTL 交互能力：**交互层不自己执行开发任务**（占用主会话）——一次完成 job 提交后**继续处理其他事物**。开发任务在 PTH 任务池异步执行，交互层需要时取结果。性能数据伴随采集（V8 引擎专项优化的铺垫）。
...
└─────────────────────────────────────────────────────┘
        ↓ 提交（HTTP POST /api/v1/kernel/jobs）
┌─ PTH（服务器端任务池）──────────────────────────────┐
...
交互层可同时提交多个 job（不同计划）——每个独立 jobId——PTH 任务池并行执行——交互层按需逐个 fetch（主会话零阻塞）。

## 相关

- 任务分配：正交角色谱系（docs/pth/kernel.md）——flow/tags/hash 三段路由
- 性能自愈：PerfAutopilot（循环②——参数自动优化）
- v0.9 方向：交互层升级（不自己开发任务）+ V8 引擎专项优化
