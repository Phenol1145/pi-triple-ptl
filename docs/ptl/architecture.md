...
┌──────────────────────────────────────────────────────────────┐
│                      pit CLI（入口）                         │
│  ptl onboard/start/pi/tui/hub/template/config/doctor/install… │
├──────────────────────────────────────────────────────────────┤
│  pit TUI (Ink)               │  pi 进程 × N (tmux)          │
│  ┌──────────┐ ┌────────────┐ │  ┌────────┐ ┌────────┐      │
│  │ ptl tui  │ │ lab ui     │ │  │ process│ │ process│ ...  │
│  │ 控制面板  │ │ 模型调试   │ │  │ tmpl   │ │ tmpl   │      │
│  └──────────┘ └────────────┘ │  │ A      │ │ B      │      │
├──────────────────────────────┤  └────────┘ └────────┘      │
│  共享层 (shared/)             │                              │
...
| 分组 | 命令 | 说明 |
|------|------|------|
| **启动** | `ptl start` / `ptl pi` | tmux 管理模式 / 原生前台 |
| | `ptl start --bg --name <n>` | 纯后台 tmux |
| **会话** | `ptl ls` / `ptl attach` / `ptl stop` | 列出/接入/停止会话 |
| | `ptl switch` / `ptl detach` | 瞬移/脱离（tmux 内） |
| **TUI** | `ptl tui dashboard` | 系统总控 TUI |
| | `ptl tui lab` | agent-lab 模型调试 TUI |
| **模板** | `ptl template ls/new/rm/rename` | UUID + alias 管理（agent 配置蓝图） |
| **配置** | `ptl config/get/set/unset` | 读写 pi-triple.json |
| **扩展** | `ptl install/remove/update` | pi 扩展管理 |
| | `ptl shared status/init` | 共享层操作 |
| **运维** | `ptl onboard/doctor/status` | 导引/诊断/状态 |
| | `ptl migrate` | 从 ~/.pi/agent 迁移 |
| **流程** | `ptl flow run/status/show/ls` | ptl-flow 波次工作流引擎 |
| | `ptl flow approve/reject/resume` | human gate 审批 / 恢复 |
| | `ptl flow set/edit/propose/discard` | 运行中热修改（停波护栏） |
| **桥** | `ptl hub submit/run/dev` | PTL→PTH agent 程序提交/运行 |
| | `ptl hub programs` | 列出 PTH 上的程序 |

### 模式分辨

```
ptl start                      → print 模式（直接启动 pi/tmux）
ptl config get redis            → print 模式（输出纯文本值）
ptl config get redis --json     → JSON 模式（`{"ok":true,"data":"redis://..."}`）
ptl tui dashboard               → 交互 TTY → 系统总控 TUI
                                 → 非 TTY   → "TUI 需要交互式终端"
pit                             → 打印上手指引
```

所有参数解析在 `src/ptl/cli/args.ts`，模式路由在 `src/ptl/cli/mode.ts`。

## tmux 会话生命周期

```
ptl start --template local
  │
  ...
  │
  ├─ tmux new-session -s ptl-<name> -c <cwd>
...
关键模块：`src/ptl/tmux.ts`（会话命名/构建/列表/存活检查）、`src/ptl/launcher.ts`（pi 启动参数构建）、`src/ptl/cli/sessions.ts`（start/pi/attach/switch/detach 命令）。
...
└── pi-config/<uuid>/
    └── extensions/
        ├── ptl-providers → ../../../shared/extensions/ptl-providers
        ├── ptl-communicate → ../../../shared/extensions/ptl-communicate
        ├── ptl-control → ../../../shared/extensions/ptl-control
...
- `syncBundledExtensions(sharedDir)` — `ptl update --all` 覆盖式同步（平台托管）
...
├── providers.json           ← provider 声明（ptl-providers 扩展消费）
└── data/
    ...
    └── flows/<runId>/       ← ptl-flow 运行态（graph/meta/state/checkpoints/waves/locks）
```

配置驱动：`src/ptl/config.ts`，全局查找路径由 `PI_TRIPLE_HOME` 或 `ptlHome()` 决定（默认 `~/.pi-triple`）。
...
- **收发**：`/ptl send <name> <msg>` / `/ptl ask <name> <q>` / `/ptl inbox`
- **审核模式**：`manual`（默认，人工 `/ptl accept`）/ `auto`（自动注入下轮）/ `hybrid`
- **会话注册**：每个 pi 进程启动时自动注册到 `registry.json`
- **文件分享**：`/ptl share <name> <file>`（store-and-forward）
...
在 pi 会话里直接驱动 ptl-flow 引擎（shell 调 `ptl flow` CLI，不 import src/）。
...
| **Human gate** | `type:"human"` 节点写 pending.json 暂停；`ptl flow approve/reject` 应用 writes + 推进 firedEpoch + 恢复 |
...
ptl hub dev ./my-agent      ← 本地直跑（pipe.ts 注入 systemPrompt+skills 到 pi 启动参数）
ptl hub programs            ← 列出 PTH 上的程序
```
...
`src/ptl/lab-data/` —— `ptl tui lab` TUI 的数据底座。SQLite（WAL + busy_timeout）：共享 `runs` DB（跨模板 LLM 调用遥测）+ per-template arena/events/config。模块：`telemetry.ts` · `arena.ts` · `events.ts` · `open-db.ts` · `schema.ts`。
...
| TUI | 命令 | Tab 主题 |
|-----|------|---------|
| ptl tui | `ptl tui dashboard` | Dashboard / Templates / Sessions / Extensions / Config |
| lab ui | `ptl tui lab` | Telemetry / Arena / Events / Compare / Config |
...
共享组件：`Screen`（布局）、`DataTable`（表格）、`SelectList`（列表选择）、`ConfirmDialog`（确认框）、`SparkLine`/`BarChart`（图表）、`useTabs`/`useRefresh`/`useTerminalSize`（hooks）、`CommandBar`（层级渐进式命令补全——ptl tui 和 lab ui 共享）。
...
pit CLI                      HTTP/SSE/WebSocket
        ...
        ───── 桥（已实现）─────
   ptl hub submit/run → PTH ProgramStore 运行
```

PTL 生产的 agent 程序（`agent.json` + skills）可通过 `ptl hub submit` 打包提交到 PTH，`ptl hub run` 以联邦模式运行（SSE 流式回显）。PTH 提供集中治理、审计、弹性伸缩，PTL 提供本地开发/调试体验。桥的实现见上文「PTL→PTH 桥」节。
