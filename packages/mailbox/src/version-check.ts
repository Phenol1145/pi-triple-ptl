/**
 * mailbox/version-check — ptl 本体 + pi SDK 更新检查（扩展侧，会话内提示用）
 *
 * 随包副本（自 extensions/_shared/version-check.ts 复制，Task 1 迁移）：
 * 与 packages/shared/src/version-check.ts（CLI 侧）独立实现，共享缓存格式。
 */
