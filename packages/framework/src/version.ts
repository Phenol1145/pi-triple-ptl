 ...
 * 故从模块位置向上探测最近一个 name 命中仓库根（@away_from/pi-triple-ptl 为拆仓后根名，
 * 旧名 @away_from/pi-triple / pi-triple 兼容）的 package.json，两种布局均命中仓库根。
 */
export function resolveRepoRootPackageJson(moduleUrl: string): { name?: string; version?: string } | null {
  ...
  for (let i = 0; i < 8; i++) {
    try {
      ...
      if (pkg.name === "@away_from/pi-triple-ptl" || pkg.name === "@away_from/pi-triple" || pkg.name === "pi-triple") return pkg;
    } catch {
